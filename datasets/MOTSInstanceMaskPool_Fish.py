...
def getPairs(id, split):
    label_root = os.path.join(fishRoot_split, split+"_instances/", "%05d"%id)
    image_root = label_root.replace('instances', 'frames')
    ...
    for ind, image_path in enumerate(image_list):
        infos.append({'name': "%05d"%id + '_' + "%05d"%ind + '.pkl', 'img': image_list[ind], 'label': label_list[ind]})
...
fishRoot = '/home/ubuntu/git-repo/VideoAmodal/dataset/data/'

split = 'val'
fishRoot_split = os.path.join(fishRoot, split+'_data')
split_vid_count = 1000
outF = os.path.join(fishRoot_split, 'ImgTrackEnvDB')
remove_and_mkdir(outF)
for i in range(split_vid_count):
    print (i)
    getPairs(i, split)
